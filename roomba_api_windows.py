"""
Windows laptop version of roomba_api.py.

Differences from the Raspberry Pi version (roomba_api.py):
- Default camera device: 0 (built-in laptop webcam)
- Default serial port:   COM3 (Arduino on Windows uses COMx)

Usage:
    python roomba_api_windows.py
    # or:
    uvicorn roomba_api_windows:app --host 0.0.0.0 --port 8000 --reload
"""
...
app = FastAPI(title="Roomba Serial API (Windows)")
...
class UsbCameraStreamer:
    def __init__(self):
        ...
        self.device = 0  # 0 = built-in laptop webcam
        self.width = 640
        self.height = 480
        self.fps = 15
        self.quality = 70
...
class ConnectionConfig(BaseModel):
    port: str = "COM3"  # Windows: Arduino shows up as COMx
    baud_rate: int = 9600
...
if __name__ == "__main__":
    import uvicorn
    print("Starting server (Windows)... Access at http://localhost:8000")
